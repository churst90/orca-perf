...
# Translators: Orca has an OCR (optical character recognition) feature that
# captures the focused window's pixels, recognizes the text via Tesseract,
# and presents the result as a virtual buffer the user navigates with
# NumPad keys. This command toggles OCR mode on the focused window.
OCR_TOGGLE = _("Toggle OCR mode on the focused window")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the previous line.
OCR_PREVIOUS_LINE = _("In OCR mode, go to the previous line")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the next line.
OCR_NEXT_LINE = _("In OCR mode, go to the next line")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the previous word.
OCR_PREVIOUS_WORD = _("In OCR mode, go to the previous word")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the next word.
OCR_NEXT_WORD = _("In OCR mode, go to the next word")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the previous
# character.
OCR_PREVIOUS_CHARACTER = _("In OCR mode, go to the previous character")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the next character.
OCR_NEXT_CHARACTER = _("In OCR mode, go to the next character")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the first word of
# the buffer.
OCR_FIRST_WORD = _("In OCR mode, go to the first word of the buffer")

# Translators: In OCR mode, the user navigates a virtual buffer of
# recognized text. This command moves the OCR cursor to the last word of
# the buffer.
OCR_LAST_WORD = _("In OCR mode, go to the last word of the buffer")

# Translators: In OCR mode, this command re-speaks the word currently under
# the OCR cursor.
OCR_SPEAK_CURRENT_WORD = _("In OCR mode, re-speak the current word")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one line backward.
OCR_SELECT_PREVIOUS_LINE = _("In OCR mode, extend selection to the previous line")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one line forward.
OCR_SELECT_NEXT_LINE = _("In OCR mode, extend selection to the next line")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one word backward.
OCR_SELECT_PREVIOUS_WORD = _("In OCR mode, extend selection to the previous word")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one word forward.
OCR_SELECT_NEXT_WORD = _("In OCR mode, extend selection to the next word")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one character backward.
OCR_SELECT_PREVIOUS_CHARACTER = _("In OCR mode, extend selection to the previous character")

# Translators: In OCR mode, holding Shift while pressing a navigation key
# extends the active text selection. This command extends the selection by
# one character forward.
OCR_SELECT_NEXT_CHARACTER = _("In OCR mode, extend selection to the next character")

# Translators: In OCR mode, this command marks the current OCR cursor
# position as the start of a text selection. Subsequent navigation
# extends the selection from that anchor through the new cursor position.
OCR_SET_ANCHOR = _("In OCR mode, mark the selection anchor at the OCR cursor")

# Translators: In OCR mode, this command copies the active text selection
# to the system clipboard, or the current line if no selection is active.
OCR_COPY_SELECTION_OR_LINE = _("In OCR mode, copy the selection or current line to clipboard")

# Translators: In OCR mode, this command synthesizes a left mouse click at
# the screen position of the word currently under the OCR cursor. The
# click is delivered to the source window the OCR was performed on.
OCR_LEFT_CLICK = _("In OCR mode, left-click on the word at the OCR cursor")

# Translators: In OCR mode, this command synthesizes a right mouse click at
# the screen position of the word currently under the OCR cursor.
OCR_RIGHT_CLICK = _("In OCR mode, right-click on the word at the OCR cursor")

# Translators: This command exits OCR mode, restoring the keybindings that
# were temporarily suspended while OCR mode was active.
OCR_EXIT_MODE = _("Exit OCR mode")
